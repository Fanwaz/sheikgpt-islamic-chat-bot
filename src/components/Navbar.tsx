    ...
    <header className="sticky top-0 z-50 w-full backdrop-blur-lg bg-background/80 border-b border-border transition-all duration-300 ease-in-out">
      <div className="container flex h-14 md:h-16 items-center justify-between px-3 md:px-6 lg:px-8">
        {isMobile ? (
          <>
            {user ? (
              <button className="relative flex h-9 w-9 items-center justify-center rounded-full bg-primary text-primary-foreground hover:bg-primary/90 transition-all duration-300 ease-in-out">
                <span className="font-medium text-xs">{user.initials}</span>
              </button>
            ) : (
              <Link to="/signin" className="text-xs font-medium">
                Sign in
              </Link>
            )}
